...
module.exports.run = async(client, message, args) => {
	...
	if(!args[0]) {
		return message.channel.send("Por favor digite da forma correta")
	}
	...
	if(rolls == ""){ // se nao tiver rolls
		var rolls = 1
	}

	if(sides <= 0) {
		return message.channel.send("Esqueceu de digitar quantas faces tem o dado")
	}

	if(isNaN(rolls) || isNaN(sides))  {
		return message.channel.send("Digite números")
	}

	if(rolls > 99 || sides > 200) {
		return message.channel.send("Esse número é muito grande pra um dado")
	}
